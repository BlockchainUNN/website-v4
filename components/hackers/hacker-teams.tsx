...
const formSchema = z.object({
  teamName: z.string().min(2, {
    message: "Enter a valid team name or code (minimum 2 characters)",
  }),
});
...
export default function HackerTeam() {
  ...
  const onSubmit = async (data: FormValues) => {
    if (activeTab === "create") {
      ...
      const payload: JoinTeamForm = {
        inviteCode: data.teamName,
      };
      joinTeam(payload, {
        onSuccess: () => {
          reset();
  ...
  return (
    <div className="bg-[url('/assets/events/hero_bg.jpg')] bg-cover bg-center w-full md:w-[65vw] h-full pt-16 pb-16 space-y-12">
      ...
      <section className="w-10/12 h-fit mx-auto bg-white outline-5 outline-black p-6 lg:p-10 flex flex-col items-center justify">
        <div className="hidden lg:flex w-10/12 flex-col lg:flex-row items-center justify-center gap-10 mx-auto">
          <StackGridButton
            ...
            bgColor={
              activeTab === "create"
                ? "linear-gradient(180deg, #02641C 0%, #02641C 100%)"
                : "#E0E0E0"
            }
            borderColor="#024539"
            size="large"
            ...
            arrowIcon={
              <UserPlus2Icon
                className={cn({ "text-[#E0E0E0]": activeTab === "join" })}
              />
            }
          />
          <StackGridButton
            ...
            arrowIcon={
              <UsersRoundIcon
                className={cn({ "text-[#E0E0E0]": activeTab === "create" })}
              />
            }
          />
        </div>
